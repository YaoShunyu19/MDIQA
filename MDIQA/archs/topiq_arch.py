...
class CFANet(nn.Module):
    ...
    def forward_cross_attention(self, x, y=None):
        ...
        final_feat = final_token.permute(1, 2, 0).reshape(b, -1, th, tw)
        out_score = self.score_linear(final_token.mean(dim=0))  # B,1

        return out_score, query_feat
    ...
    def forward(self, x, y=None, if_quality_map=False):
        ...
        if self.crops > 1 and not self.training:
            ...
            score, regional_feat = self.forward_cross_attention(x, y)
            score = score.reshape(bsz, self.crops, self.num_class)
            score = score.mean(dim=1)
        else:
            score, regional_feat = self.forward_cross_attention(x, y)
        # return [score], [regional_feat]
        return score, regional_feat, score