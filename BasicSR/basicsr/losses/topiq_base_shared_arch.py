...
from .topiq_arch import CFANet, GatedConv, TransformerEncoder, TransformerEncoderLayer, TransformerDecoder, TransformerDecoderLayer
...
class CFANet_base_shared(nn.Module):
    def __init__(self,
        ...
        assert semantic_model_name in ['resnet50', 'mobilenetv3_large_100.ra_in1k']
        ...
        self.dim_reduce_modules = nn.ModuleList([copy.deepcopy(self.dim_reduce) for _ in range(self.num_class + self.quality_map)])
        self.score_linear_modules = nn.ModuleList([copy.deepcopy(self.score_linear) for _ in range(self.num_class)])
        self.weight_linear_modules = nn.ModuleList([copy.deepcopy(self.weight_linear) for _ in range(self.num_class)])

        del self.dim_reduce
        del self.score_linear
        del self.weight_linear

        if self.quality_map > 0:
            ...
            self.quality_map_linear_modules = nn.ModuleList([copy.deepcopy(self.quality_map_linear) for _ in range(self.quality_map)])
            del self.quality_map_linear
    ...
    def forward_single_dim(self, x, y=None, idx=-1, quality_map_idx=-1):
        ...
        global_score, local_score, quality_map = None, None, None
        if idx >= 0:
            tmp_dist_feat_list = []
            for i in range(start_level, end_level):
                tmp_feat = dist_feat_list[i]
                tmp_feat = self.dim_reduce_modules[idx][i](tmp_feat)
            ...
            score = self.score_linear_modules[idx](dist_feat)  # B,h,w,1
            weight = self.weight_linear_modules[idx](dist_feat)  # B,h,w,1
            global_score = torch.mean(weight * score, dim=[-3, -2]) / (torch.mean(weight, dim=[-3, -2]) + self.eps)
            local_score = torch.mean(weight * score, dim=[-1])
            # B,1  B,h,w

        if self.quality_map > 0 and quality_map_idx >= 0:
            tmp_dist_feat_list = []
            for i in range(start_level, end_level):
                tmp_feat = dist_feat_list[i]
                tmp_feat = self.dim_reduce_modules[self.num_class + quality_map_idx][i](tmp_feat)
                if tmp_feat.shape[2] != th and tmp_feat.shape[3] != tw:
                    tmp_feat = F.adaptive_avg_pool2d(tmp_feat, (th, tw))
                tmp_dist_feat_list.append(tmp_feat)
            dist_feat = torch.cat(tmp_dist_feat_list, dim=1)  # B,5c,h,w
            dist_feat = dist_feat.permute(0, 2, 3, 1)
            quality_map = self.quality_map_linear_modules[quality_map_idx](dist_feat)[:, :, :, 0]  # B,h,w

        return global_score, local_score, quality_map

    def forward_multi_dim(self, x, y=None, tmp_img_feat=None):
        ...
        if tmp_img_feat is not None:
            tmp_img_feat = dist_feat_list
        ...
        global_scores, local_scores, quality_maps = [], [], []
        for idx in range(self.num_class):
            ...
            weight = self.weight_linear_modules[idx](dist_feat)  # B,h,w,1
            global_score = torch.mean(weight * score, dim=[-3, -2]) / (torch.mean(weight, dim=[-3, -2]) + self.eps)
            local_score = torch.mean(weight * score, dim=[-1]) / (torch.mean(weight, dim=[-3, -2]) + self.eps)
            # B,1  B,h,w
            global_scores.append(global_score)
            local_scores.append(local_score)
        global_scores = torch.stack(global_scores, dim=1)[:, :, 0]  # B,N
        local_scores = torch.stack(local_scores, dim=1)  # B,N,h,w

        if self.quality_map > 0:
            for idx in range(self.quality_map):
                tmp_dist_feat_list = []
                for i in range(start_level, end_level):
                    tmp_feat = dist_feat_list[i]
                    tmp_feat = self.dim_reduce_modules[-1][i](tmp_feat)
                    if tmp_feat.shape[2] != th and tmp_feat.shape[3] != tw:
                        tmp_feat = F.adaptive_avg_pool2d(tmp_feat, (th, tw))
                    tmp_dist_feat_list.append(tmp_feat)
                dist_feat = torch.cat(tmp_dist_feat_list, dim=1)  # B,5c,h,w
                dist_feat = dist_feat.permute(0, 2, 3, 1)
                quality_map = self.quality_map_linear_modules[idx](dist_feat)[:, :, :, 0]  # B,h,w
                quality_maps.append(quality_map)
            quality_maps = torch.stack(quality_maps, dim=1)  # B,Nq,h,w

        return global_scores, local_scores, quality_maps, tmp_img_feat

    def forward(self, x, y=None, idx=0, quality_map_idx=-1, tmp_img_feat=None):
        if self.use_ref:
        ...
        if idx == 'multi':
            global_scores, local_scores, quality_map, tmp_img_feat = self.forward_multi_dim(x, y, tmp_img_feat)  # B,N  B,N,h,w  B,h,w, []
            return global_scores, local_scores, quality_map, tmp_img_feat
        else:
            global_scores, local_scores, quality_map = self.forward_single_dim(x, y, idx, quality_map_idx)  # B,1  B,h,w  B,h,w
            return global_scores, local_scores, quality_map


class InstanceSpecificWeightedBranch(nn.Module):
    def __init__(self,
                 semantic_model_name='resnet50',
                 model_name='cfanet_base_nr_koniq_res50',
                 backbone_pretrain=True,
    ...
    def forward(self, x, y=None):
        ...
        dist_feat = torch.cat(tmp_dist_feat_list, dim=1)  # B,5c,h,w
        dist_feat = dist_feat.permute(0, 2, 3, 1)
        score = self.score_linear(dist_feat)  # B,h,w,C
        weight = torch.mean(score, dim=[-3, -2])  # B,C
        return torch.sigmoid(weight)


class GlobalBranch(nn.Module):
    def __init__(self, semantic_model_name, inter_dim, act_layer):
        super().__init__()
        semantic_model = timm.create_model(semantic_model_name, pretrained=True, features_only=True)
    ...
    def forward(self, tmp_img_feat):
        start_level = 0
        end_level = len(tmp_img_feat)   # [B,ci,384/2^i,384/2^i]

        _, _, th, tw = tmp_img_feat[end_level - 2].shape  # TODO

        global_score, local_score = None, None

        tmp_dist_feat_list = []
        for i in range(start_level, end_level):
            tmp_feat = tmp_img_feat[i]
            tmp_feat = self.dim_reduce[i](tmp_feat)
            if tmp_feat.shape[2] != th and tmp_feat.shape[3] != tw:
                tmp_feat = F.adaptive_avg_pool2d(tmp_feat, (th, tw))
            tmp_dist_feat_list.append(tmp_feat)
...
class SharedMultiDimFusionWeightedMLP(nn.Module):
    def __init__(self,
                 num_dim=1,
                 inter_dim=18,
                 activation='gelu',
        ...
        if self.instance_specific_weighted:
            self.instance_specific_weighted_branch = InstanceSpecificWeightedBranch(use_ref=False,
                                                                                    quality_map=False,
                                                                                    num_class=num_dim)

        self.global_branch = global_branch
        if self.global_branch > 0:
            global_branch_module = GlobalBranch(semantic_model_name, inter_dim, self.act_layer)
            self.global_branch_modules = nn.ModuleList([copy.deepcopy(global_branch_module) for _ in range(global_branch)])
        ...
        for m in self.net:
            if isinstance(m, nn.Linear):
                ...
                if m.bias is not None:
                    init.normal_(m.bias, mean=0.0, std=0.1)
                    m.bias.data = torch.abs(m.bias.data)

    def forward(self, img, tmp_img_feat, scores, weights_idx=0):
        # tmp_img_feat: [[], [], ..., []]
        # scores: B,d
        if self.global_branch > 0:
            for i in range(self.global_branch):
                global_branch_score, _ = self.global_branch_modules[i](tmp_img_feat[i])  # B,1
                scores = torch.cat([scores, global_branch_score], dim=1)  # B,d

        if self.instance_specific_weighted:
            # 1,d + B,d
            weight = self.dataset_specific_weights[weights_idx][None, ...] + self.instance_specific_weighted_branch(img)
        else:
            weight = self.dataset_specific_weights[weights_idx][None, ...]

        weighted_scores = scores * weight  # B,d * B,d
        output_score = self.net(weighted_scores)
        return output_score
